...
let model_namespace = 'VaahCms\\Modules\\CustomerChannel\\Models\\User';


let base_url = document.getElementsByTagName('base')[0].getAttribute("href");
let ajax_url = base_url + "/customerchannel/users";
...
export const useUserStore = defineStore({
    ...
    actions: {
        ...
        async afterGetList (data, res) {
            ...
            this.total_product=res.data.totalProduct;
            if (data) {
                this.list = data;
        ...
        async getUserProducts () {
            ...
            let url = this.ajax_url+'/item/' + this.item.id + '/products';
            ...
            let params = {
             ...
             vaah().ajax(
                url,
                await this.afterGetUserProducts,
                params
            );
        },
        //---------------------------------------------------------------------
        async afterGetUserProducts(data, res) {
            this.hideProgress();
            if (data) {
                this.user_roles = data;
                this.selected_product_ids = data.matchingProductIds;
            }
        },
        //---------------------------------------------------------------------
        ...
        async changeUserRole(item,id){
            ...
            await this.actions(false, 'toggle-product-active-status', params, data)

        },
        //---------------------------------------------------------------------
        async bulkActions (input, action) {
            let ids = this.selected_product_ids;
            let params = {
                id: this.item.id,
                product_id: ids
            };
        ...
        async getUserRolesMenuItems() {
            return this.user_roles_menu = [
                {
                    label: 'Active All Products',
                    command: async () => {
                        await this.bulkActions(1, 'toggle-all-product-active-status')
                    }
                },
                {
                    label: 'Inactive All Products',
                    command: async () => {
                        await this.bulkActions(0, 'toggle-all-product-active-status')
                    }
                },
            ]
        },
