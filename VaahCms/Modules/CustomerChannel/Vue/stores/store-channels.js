...
let model_namespace = 'VaahCms\\Modules\\CustomerChannel\\Models\\Channel';


let base_url = document.getElementsByTagName('base')[0].getAttribute("href");
let ajax_url = base_url + "/customerchannel/channels";
...
export const useChannelStore = defineStore({
    ...
    actions: {
        ...
        //---------------------------------------------------------------------
        async getName() {
            const data = await vaah().ajax(
                this.ajax_url + '/customer-names',
                this.afterGetName
            )
        },
        async afterGetName(data, res) {
            if (data) {
                this.customer_names = data;
        ...
        handleCustomerSelect() {
            this.item.customer_id = this.item.customers.id;
        },
        //---------------------------------------------------------------------
        setCustomerId(route) {
            if (this.query.customer_id || route.query.customer_id) {
                let id = Number(this.query.customer_id || route.query.customer_id);
                if (this.assets.customers && this.assets.customers.data) {
                    this.item.customers = this.assets.customers.data.find((customer) => customer.id === id)
                }
            }
        },
        //---------------------------------------------------------------------
        setCustomerSlug(route) {
            if (this.query.filter && this.query.filter.customer && this.query.customer_slug) {
                this.query.filter.customer = [];
                this.query.filter.customer.push(route.query['customer_slug']);
                this.query['customer_slug'] = [];
            }
        },
        //---------------------------------------------------------------------
