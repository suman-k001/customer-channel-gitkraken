...
let model_namespace = 'VaahCms\\Modules\\CustomerChannel\\Models\\Customer';


let base_url = document.getElementsByTagName('base')[0].getAttribute("href");
let ajax_url = base_url + "/customerchannel/customers";
...
export const useCustomerStore = defineStore({
    ...
    actions: {
        ...
        //---------------------------------------------------------------------
        toChannels(item) {
            this.$router.push({name: 'channels.index', query: {customer_slug: item}});
        },
        toChannelsForm(id) {
            this.$router.push({name: 'channels.form', query: {customer_id: id}});
        },
        //---------------------------------------------------------------------
        toEdit(item) {
            this.item = item;
        ...
        async fetchCountry() {
            axios.get('https://restcountries.com/v3.1/all?fields=name').then((response) => {
                this.countries.row = response.data.map((country) => country.name.common)
            })
        },
        search(event) {
            const max_suggestions = 5
            this.countries.name = this.countries.row
