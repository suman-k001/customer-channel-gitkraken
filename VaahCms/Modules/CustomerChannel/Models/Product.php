...
class Product extends Model
{
    ...
    public function users(): BelongsToMany
    {
        return $this->belongsToMany(
            User::class, 'vh_user_products',
            'product_id',
            'user_id')->withPivot('is_active',
            'created_by',
            'created_at',
            'updated_by',
            'updated_at');
    ...
    {
        return self::getProdcutUser($request, $id);
    }
    ...
    public static function countUsers($id): int
    {
        ...
        return $products->users()->wherePivot('is_active', 1)->count();
    }

    public static function getProdcutUser($request, $id)
    {
    ...
    public static function getList($request)
    {
        ...
        $list->withCount(['activeUsers']);
        $list = $list->paginate($rows);
        $response['totalUser'] = User::all()->count();
        $response['success'] = true;
        $response['data'] = $list;
        return $response;
    ...
    public static function changeUserStatus($request): array
    {
        ...
        if ($inputs['inputs']['user_id']) {
            ...
            $item->users()->updateExistingPivot(
                $inputs['inputs']['user_id'],
                $data
            );
        } else {
            $item->users()
    ...
    public static function bulkChangeUserStatus($request): array
    {
        ...
        if (isset($inputs['id'])) {
            ...
            foreach ($userIds as $userId) {
                ...
                $item->users()->updateExistingPivot($userId, $updateData);
            }
        }
